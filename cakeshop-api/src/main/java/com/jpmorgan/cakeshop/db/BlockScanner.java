...
/**
 * Service which tracks the Blockchain in a local database. As blocks are added to the chain, they are added to our database in
 * near-realtime.
 *
 * @author Chetan Sarva
 *
...
public class BlockScanner extends Thread
{
	...
	protected Long backfillBlocks()
	{
		...
		if (largestSavedBlock == null)
		{
			...
			if (chainBlock.getNumber().longValue() > largestSavedBlock.getNumber().longValue())
			{
				...
				if (chainBlock.equals(largestSavedBlock))
				{
	...
	private void handleChainReorg()
	{
		...
		Long maxDBBlock = findMaxDatabaseBlock();
		if (maxBlock >= 0)
		{
			while (maxDBBlock < maxBlock)
			{
				maxDBBlock = findMaxDatabaseBlock();
	 ...
	 * @return
	 */
	private long findMaxDatabaseBlock()
	{
		Block latest = blockDAO.getLatest();
		return (latest == null) ? 0 : latest.getNumber().longValue();
	}
