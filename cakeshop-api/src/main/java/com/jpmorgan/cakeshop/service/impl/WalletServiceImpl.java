...
public class WalletServiceImpl implements WalletService, GethRpcConstants
{
	...
	private static final String DUMMY_PAYLOAD_HASH = HexUtils.toHexString(AbiUtils.sha3AsHex("foobar"));
	...
	public List<Account> list() throws APIException
	{
		...
		if (data != null && data.containsKey("_result"))
		{
			...
			if (accountList != null)
			{
				...
				{
					Map<String, Object> accountData = gethService.executeGethCall(PERSONAL_GET_ACCOUNT_BALANCE, new Object[] { address,
							"latest" });
	...
	public Boolean unlockAccount(WalletPostJsonRequest request) throws APIException
	{
		try
		{
			Map<String, Object> result = gethService.executeGethCall("personal_unlockAccount",
					new Object[] { request.getAccount(), request.getAccountPassword() });
	...
	public boolean isUnlocked(String address) throws APIException
	{
		try
		{
			...
			{
				LOG.info("Unlocked Address = " + address);
				return true;
			}
		}
